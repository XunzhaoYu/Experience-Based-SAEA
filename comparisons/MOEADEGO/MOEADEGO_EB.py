...
class MOEADEGO:
    ...
    def variable_init(self, current_iteration):
        ...
        # --- variable initialization for meta-learning ---
        self.q_phi_train = []
        self.q_mu = []
        self.q_sigma2 = []
        self.q_invR_ycen = []
    ...
    def _surrogate_init(self, X, Y, b_exist):  # ***
        ...
        for obj in range(self.n_objs):
            g = tf.Graph()
            s = tf.Session(config=tf.ConfigProto(log_device_placement=False), graph=g)
            new_surrogate = MDKL(MDKL_config, s, g)
            self.surrogates.append(new_surrogate)
            self.surrogates[obj].construct_model()
            if b_exist:
                self.surrogates[obj].restore(self.name + '-' + str(obj + 1))
            else:
                self.surrogates[obj].train(self.dataset, self.N_UPDATE, b_save=True, obj=obj)

            # first adaptation.
            self.surrogates[obj].parameter_init()  # initialize delta_theta, delta_p
            temp_phi_train, temp_mu, temp_sigma2, temp_invR_ycen, temp_chol_L = \
                self.surrogates[obj].adapt([X], Y[:, obj].reshape(1, self.EVALUATION_INIT, 1), b_speed_up=True)
            self.q_phi_train.append(temp_phi_train)
            self.q_mu.append(temp_mu)
    ...
    def _surrogate_fit(self, X, Y, c_size):  # ***
        ...
        X_size = len(X)
        MSEs = np.zeros(X_size)
        for obj in range(self.n_objs):
            self.surrogates[obj].save('temp/' + self.name + '-' + str(self.archive_size) + '-' + str(obj + 1))

            temp_phi_train, temp_mu, temp_sigma2, temp_invR_ycen, temp_chol_L = self.surrogates[obj].no_adapt([X], Y[:, obj].reshape(1, X_size, 1))
            for i in range(X_size):
                y_hat = self.surrogates[obj].predict(temp_phi_train, temp_mu, temp_sigma2, temp_invR_ycen, temp_chol_L, X[i, :].reshape(1, 1, -1), b_sigma=False)
                MSEs[i] = np.square(y_hat - Y[i, obj])
            MSE_no_adapt = np.mean(MSEs)

            temp2_phi_train, temp2_mu, temp2_sigma2, temp2_invR_ycen, temp2_chol_L = self.surrogates[obj].adapt([X], Y[:, obj].reshape(1, X_size, 1), b_speed_up=True)
            for i in range(X_size):
                y_hat = self.surrogates[obj].predict(temp2_phi_train, temp2_mu, temp2_sigma2, temp2_invR_ycen, temp2_chol_L, X[i, :].reshape(1, 1, -1), b_sigma=False)
                MSEs[i] = np.square(y_hat - Y[i, obj])
            MSE_adapt = np.mean(MSEs)

            if MSE_no_adapt < MSE_adapt:
                self.surrogates[obj].restore('temp/' + self.name + '-' + str(self.archive_size) + '-' + str(obj + 1))
                self.q_phi_train.append(temp_phi_train)
                self.q_mu.append(temp_mu)
                self.q_sigma2.append(temp_sigma2)
                self.q_invR_ycen.append(temp_invR_ycen)
                self.q_chol_L.append(temp_chol_L)
                print('obj {:d}: no adaptation: {:.6f} < adaptation {:.6f}'.format(obj, MSE_no_adapt, MSE_adapt))
            else:
                self.q_phi_train.append(temp2_phi_train)
                self.q_mu.append(temp2_mu)
                self.q_sigma2.append(temp2_sigma2)
                self.q_invR_ycen.append(temp2_invR_ycen)
    ...
    def _surrogate_predict(self, x, MSE=True):  # ***
        ...
        if MSE:
            sigma2_hat = np.zeros((self.n_objs,), dtype=float)
            for obj_index in range(self.n_objs):
                mu_hat[obj_index], sigma2_hat[obj_index] = self.surrogates[obj_index].predict(
                    self.q_phi_train[obj_index], self.q_mu[obj_index], self.q_sigma2[obj_index], self.q_invR_ycen[obj_index],
                    self.q_chol_L[obj_index], np.array(x).reshape(1, 1, -1), b_sigma=True)
            return mu_hat, sigma2_hat
        else:
            for obj_index in range(self.n_objs):
                mu_hat[obj_index] = self.surrogates[obj_index].predict(
                    self.q_phi_train[obj_index], self.q_mu[obj_index], self.q_sigma2[obj_index], self.q_invR_ycen[obj_index],
                    self.q_chol_L[obj_index], np.array(x).reshape(1, 1, -1), b_sigma=False)
            return mu_hat
    ...
    def _progress_update(self):
        ...
        # print results
        t = time() - self.time
        print("MOEA/D-EGO(EB), Evaluation Count: {:d}.  Total time: {:.0f} mins, {:.2f} secs.".format(self.archive_size, t // 60, t % 60))
